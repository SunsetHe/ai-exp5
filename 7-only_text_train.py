...
print(train_jpg_vector[0].shape)
print(train_jpg_vector[0])
print(len(train_jpg_vector))

# TODO 构造一个新的训练集的text的特征向量，但是让被拼接的text的向量变为空向量，要求形状与原来一样

new_train_jpg_vector = [torch.zeros_like(jpg_vec) for jpg_vec in train_jpg_vector]
train_jpg_vector = new_train_jpg_vector
...
# Step 5: 保存模型
torch.save(model.state_dict(), f'train_only_text_model_epoch_{num_epochs}.pt')
